...
public class SystemCache {
	 ...
	 */
	public void init(String path) throws BusinessException {
		...
		if (!file.exists()) {
			throw new BusinessException("获取配置文件路径失败，未找到的路径:" + webConfigPath);
		}
		...
		try {
			...
			throw new BusinessException("初始化系统配置文件失败，I/O异常：", e);
		} finally {
			try {
				is.close();
			} catch (Exception e) {
