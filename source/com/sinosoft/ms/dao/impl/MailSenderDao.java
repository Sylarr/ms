 ...
 * @date 2014-10-13
 */
public class MailSenderDao extends BaseSupportDao {
	...
	public Bean read(Object... params) {
		return QueryHelper.read(MailSender.class, "SELECT * FROM " + getBean().getTableName() + " WHERE SenderId=?", params);
	}
	
	@Override
	public Bean read_cache(Object... params) {
		return QueryHelper.read_cache(MailSender.class, MailSender.class.getSimpleName(), String.valueOf(params[0]), 
	 ...
	 * @return
	 */
	private MailSender cast(Bean bean) {
		if (bean == null || !(bean instanceof MailSender)) {
			throw new BusinessException("MailSender强转参数异常!");
		}
